    ...
    return (
        <div className="max-w-4xl mx-auto py-12 px-6">
            <div className="text-center mb-12">
                <div className="bg-sky-500/10 p-4 rounded-full w-20 h-20 flex items-center justify-center mx-auto mb-6">
                    <BookOpen className="w-10 h-10 text-sky-600 dark:text-sky-400" />
                </div>
                <h1 className="text-4xl font-bold text-foreground mb-4">Wiki & Knowledge Base</h1>
                        ...
                        {recentNodes.map(node => (
                            <Link key={node.id} href={`/wiki/${node.id}`}>
                                <BasicCard
                                    ...
                                    hoverEffect={true}
                                    className="h-full border border-border shadow-sm bg-card"
                                >
                                    <div className="mt-2 text-xs font-semibold text-sky-600 dark:text-sky-400 bg-sky-500/10 inline-block px-2 py-1 rounded">
